...
func TestQueueFlattensMessages(t *testing.T) {
	...
	// the captured fixture reflects an idle instance (no active downloads);
	// message-flattening itself is covered by TestQueueFlattensMessagesFromRecord.
	if len(items) != 0 {
		t.Fatalf("queue items: expected empty fixture, got %+v", items)
	}
	if q := seen[0].URL.Query(); q.Get("includeUnknownMovieItems") != "true" || q.Get("pageSize") == "" {
		t.Errorf("query params missing: %v", q)
	}
}

func TestMoviesDecode(t *testing.T) {
	srv := newServer(t, map[string]string{"/api/v3/movie": "movie.json"}, nil)
	defer srv.Close()
	c, _ := New(srv.URL, "key")
	m, err := c.Movies(context.Background())
	if err != nil || len(m) == 0 || m[0].Title == "" {
		t.Fatalf("movies: %+v %v", m, err)
	}
	if m[0].HasFile {
		t.Errorf("expected HasFile false per fixture, got true")
	}
	if !m[0].Monitored {
		t.Errorf("expected Monitored true per fixture, got false")
	}
	if m[0].TMDBID == 0 {
		t.Errorf("expected TMDBID to decode, got 0")
	}
}
...
func TestHistoryPaginatesWhenServerReturnsFewerRecordsThanRequested(t *testing.T) {
	...
	srv := newPaginatedServer(t, "/api/v3/history", total, serverPageSize, func(i int) map[string]any {
		return map[string]any{
			...
			"date": time.Now().Format(time.RFC3339), "movieId": i + 1,
		}
	})
	defer srv.Close()
	c, _ := New(srv.URL, "key")
...
func TestQueuePaginatesWhenServerReturnsFewerRecordsThanRequested(t *testing.T) {
	...
	srv := newPaginatedServer(t, "/api/v3/queue", total, serverPageSize, func(i int) map[string]any {
		return map[string]any{"id": i + 1, "movieId": 1, "downloadId": fmt.Sprintf("dl-%d", i+1)}
	})
	defer srv.Close()
	c, _ := New(srv.URL, "key")
	items, err := c.Queue(context.Background())
