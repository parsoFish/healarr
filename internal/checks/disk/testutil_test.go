...
// exact sentinel.
var errAny = errors.New("disk_test: any error")

// baseDeps returns a check.Deps factory for table-driven tests: fixed node
// and clock, the given config and clients. Any client may be nil, mirroring
// a node where that service isn't configured.
func baseDeps(cfg config.Config, host hostfs.Client, dockerClient docker.Client, qbit qbittorrent.Client) func() check.Deps {
